...
from schemas.usuario_schama import UsuarioSchemaBase, UsuarioShemaCreate, UsuarioSchemaUp, UsuarioSchemaArtigos
...
async def post_usuario(usuario: UsuarioShemaCreate, db: AsyncSession = Depends(get_session)):
    ...
    async with db as session:
        try:
            ...
            raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail='Já existe umm usuário com esse email cadastrado.')
...
async def put_usuario(usuario_id: int, usuario: UsuarioSchemaUp, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if usuario_up:
            ...
            if usuario.senha:
                usuario_up.eh_admin = gerar_hash_senha(usuario.senha)
